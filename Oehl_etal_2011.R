...
#Primer of the trade-off spore size-spore number

# SporeOutputDisturbed<-left_join(Spore_number,AMF_All_Copy[,c(1,13)])
# SporeOutputUndisturbed<-left_join(Spore_numberUndisturbed,AMF_All_Copy[,c(1,13)])
# 
# plot(SporeOutputDisturbed$no_spores,SporeOutputDisturbed$SporeArea)
# 
# plot(log10(SporeOutputUndisturbed$SporeArea),SporeOutputUndisturbed$no_spores,
#      xlab =expression(paste("log"[10],"Spore size"," ","(",mu,"m"^2,")")),
#      ylab = "Spore output",
#      ylim = c(0,40),
#      pch=19)


#Trying to have a function with "ifelse"

# datos<-as.matrix(vegdist(as.matrix(transposed[c(8,9,15,16),-c(1:4)],method="bray")))
# datos1<-data.frame(bray.dist=(datos[,grep("Grassland",colnames(datos))][,1]))
# datos2<-data.frame(bray.dist=(datos[,grep("Grassland",colnames(datos))][,2]))
# datos1$Reference<-rownames(datos1)[datos1$bray.dist==0]
# datos1$habitat<-rownames(datos1)
...
# datos2$habitat<-rownames(datos2)
# datos<-rbind(datos1,datos2)
# #rownames(datos)<-sub("\\d$","",rownames(datos))
# 
# datos$Soil_Type<-datos$bray.dist==0


#Previos version for calculating bray curtis distances for OehlDataset

# transposed<-t(Oehl2010SporeAbundanceMatrix[,-1]);
#             transposed<-transposed[match(levels(Oehlspecies_df$sites),row.names(transposed)),]
# 
...
# 
#         dists$habitat<-factor(dists$habitat,
#                       levels = levels(Oehlspecies_df$sites));
#         
