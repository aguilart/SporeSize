...
kewData<-read.csv("kew/kew_all.csv",header = T, stringsAsFactors = F)
          kewData<-kewData[!duplicated(kewData$species),]
          kewData<-kewData[kewData$value!=0,]
          
###############
###FIGURE 1b###
###############
          
#This one refers to histogram of seed size in figure 1b
...
kewData%>%
  ...
  geom_histogram(fill=I("blue"),
                 col=I("black"), 
                 alpha=I(.2))+
labs(x="Seed size (mg) ",y="Number of species")+
  ggtitle(label="Variation in seed size among angiosperms")+
...
head(
  kewData[
    order(kewData$value,decreasing = T),])

grep("Dendrobium",kewData$species)
